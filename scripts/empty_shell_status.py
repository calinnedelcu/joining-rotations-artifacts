...
log("\nSection 7's sieve counts only the occupied-shell candidates, so these nine")
log("are settled here instead.  The periodic filter has been run on all of them")
log("(it used to refuse at an empty shell, which is what hid the survivors):")
log(f"    dead to a homomorphism colouring: {dead}")
log(f"    survive both filters, genuine candidates: {survivors}")
log("      -- each SURVIVES at modulus 4, proved, over the 66 occurring pairs")
for a in survivors:
