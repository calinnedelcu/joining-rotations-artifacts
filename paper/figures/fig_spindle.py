...
fig = plt.figure(figsize=(2.78, 2.78))
ax = fig.add_axes([0, 0, 1, 1]); ax.set_aspect("equal"); ax.axis("off")

ax.add_artist(plt.Circle((0, 0), R, fill=False, lw=0.4, ec="0.64", zorder=0))
for z in (p, q):
    ax.plot([0, z[0]], [0, z[1]], lw=0.45, color="0.42", zorder=1)
ax.plot([p[0], q[0]], [p[1], q[1]], lw=1.3, color="black", zorder=3)
ax.plot([0, h[0]], [0, h[1]], lw=1.3, color="black", zorder=3)
ax.add_artist(Arc((0, 0), 1.1, 1.1, theta1=0, theta2=degrees(atan2(sa, ca)),
                  lw=0.45, color="0.42"))
for z in (p, q, h, (0, 0)):
    ax.plot([z[0]], [z[1]], "o", ms=2.8, color="black", markeredgewidth=0)
